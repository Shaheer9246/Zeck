...
describe("chat workspace", () => {
	...
	it("creates and assigns a dedicated session workspace when sessionId is present", async () => {
		const sessionId = "session-abc-123";
		const expectedPath = resolveSessionWorkspacePath(sessionId);
		const resolved = await resolveStartSessionWorkspace(
			createConfig({ sessionId }),
		);
		expect(resolved.cwd).toBe(expectedPath);
		expect(resolved.workspaceRoot).toBe(expectedPath);

		const rules = await readFile(join(expectedPath, "AGENTS.md"), "utf8");
		expect(rules).toContain("Cline Session Workspace");
		expect(rules).toContain("belongs to ONE session");
	});

	it("sanitizes unsafe session ids in workspace path", () => {
		const path = resolveSessionWorkspacePath("session/../../etc:special?*");
