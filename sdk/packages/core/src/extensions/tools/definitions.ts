...
const RUN_COMMANDS_SHARED_INSTRUCTIONS =
	"Use for running builds, executing tests, checking git status, etc. " +
	"NEVER use shell commands (such as Get-Content, cat, type, dir, ls, find) to read files or search folders; always use read_files and search_codebase directly, which run in-process without wasting shell credits. " +
...
export function createEditorTool(
	...
	return createTool<EditFileInput, ToolOperationResult>({
		...
		description:
			...
			"Always make small, surgical edits (2-15 lines of `old_text`) rather than rewriting the entire file. Rewriting whole files wastes credits, risks token truncation, and is blocked. If several edits to different files or non-overlapping regions are already known, emit multiple editor tool calls in the same response.",
