...
const sectionDescriptions: Record<CustomizationSection, string> = {
	Rules: "Review project and global rule files that shape Zeck behavior.",
...
const sectionCommands: Record<CustomizationSection, string> = {
	Rules: "zeck config rules",
	Hooks: "zeck config hooks",
	MCP: "zeck mcp install",
	Skills: "zeck skill",
	Agents: "zeck config agents",
	Plugins: "zeck plugin",
	Tools: "zeck config tools",
};
		...
		return (
			<details key={plugin.path} className="rounded-lg border bg-card p-4">
				<summary className="flex cursor-pointer list-none items-center gap-2">
					...
					<Badge variant="outline" className="shrink-0 text-muted-foreground">
						{plugin.agentPlugin === true ? "Agent Plugin" : "Zeck Plugin"}
					</Badge>
					{context?.matchedEntries?.length ? (
						<Badge variant="outline" className="shrink-0 text-muted-foreground">
							Marketplace
