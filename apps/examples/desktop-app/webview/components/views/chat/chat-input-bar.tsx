	...
	const handleSend = useCallback(() => {
		...
		if (!prompt) {
			toast({
				...
				description:
					"Describe what you want Zeck to do with the attached files before sending.",
			});
			return;
		}
		setPromptInput("");
	...
	const handleSpeechInputError = useCallback((error: unknown) => {
		...
		toast({
			...
			description: isMicrophoneError
				? "Check the microphone permission for Zeck and try again."
				: message,
		});
	}, []);
					...
					<div className="flex shrink-0 items-center rounded-lg bg-muted/60 p-0.5 border border-border/50 text-xs font-medium">
						...
						<button
							...
							className={cn(
								"flex items-center gap-1 rounded-md px-2.5 py-1 transition-all text-xs",
								mode === "act"
									? "bg-background text-indigo-400 font-semibold shadow-xs border border-indigo-500/20"
									: "text-muted-foreground hover:text-foreground",
							)}
							onClick={() => {
