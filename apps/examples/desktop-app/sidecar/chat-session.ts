...
function readCodePanelContext(config: JsonRecord): string | undefined {
	const value =
		config.codePanelContext ??
		config.code_panel_context ??
...
async function resolveSystemPrompt(config: JsonRecord, ctx?: SidecarContext): Promise<string> {
	const cwd = String(
	...
	const inlineRules =
		typeof config.rules === "string" && config.rules.trim().length > 0
			? config.rules
			: undefined;
	let codePanelContext = readCodePanelContext(config);

	if (!codePanelContext && ctx) {
		try {
			const { getOrImportWorkspace, buildCodePanelContext } = await import(
				"./virtual-workspace"
			);
			const ws = await getOrImportWorkspace(ctx, cwd);
			codePanelContext = buildCodePanelContext(ws);
		} catch {
			// fallback without codePanelContext if import fails
		}
	}
...
async function handleStart(
	...
	const systemPrompt = await resolveSystemPrompt(request.config, ctx);
