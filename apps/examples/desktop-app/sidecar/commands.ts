...
export async function handleCommand(
	...
	// ── Virtual Workspace & Push Code Engine ────────────────────────
	if (command === "workspace_import") {
		const rootPath = String(args?.rootPath ?? args?.cwd ?? ctx.workspaceRoot).trim();
		const { importLocalProject } = await import("./virtual-workspace");
		const ws = await importLocalProject(rootPath);
		if (!ctx.virtualWorkspaces) ctx.virtualWorkspaces = new Map();
		ctx.virtualWorkspaces.set(ws.rootPath, ws);
		return {
			rootPath: ws.rootPath,
			fileCount: ws.files.size,
			stackHints: ws.stackHints,
			fileTree: ws.fileTree,
		};
	}
	if (command === "workspace_get") {
		const rootPath = String(args?.rootPath ?? args?.cwd ?? ctx.workspaceRoot).trim();
		const { getOrImportWorkspace } = await import("./virtual-workspace");
		const ws = await getOrImportWorkspace(ctx, rootPath);
		return {
			rootPath: ws.rootPath,
			activeFilePath: ws.activeFilePath,
			openFiles: ws.openFiles,
			dirtyFiles: ws.dirtyFiles,
			deletedFiles: ws.deletedFiles,
			stackHints: ws.stackHints,
			previewErrors: ws.previewErrors,
			fileTree: ws.fileTree,
		};
	}
	if (command === "workspace_context") {
		const rootPath = String(args?.rootPath ?? args?.cwd ?? ctx.workspaceRoot).trim();
		const { getOrImportWorkspace, buildCodePanelContext } = await import("./virtual-workspace");
		const ws = await getOrImportWorkspace(ctx, rootPath);
		return {
			context: buildCodePanelContext(ws),
		};
	}
	if (command === "workspace_read") {
		const rootPath = String(args?.rootPath ?? args?.cwd ?? ctx.workspaceRoot).trim();
		const filePath = String(args?.filePath ?? args?.path ?? "").trim();
		const { getOrImportWorkspace, readVirtualFile } = await import("./virtual-workspace");
		const ws = await getOrImportWorkspace(ctx, rootPath);
		return {
			content: readVirtualFile(ws, filePath),
		};
	}
	if (command === "workspace_write") {
		const rootPath = String(args?.rootPath ?? args?.cwd ?? ctx.workspaceRoot).trim();
		const filePath = String(args?.filePath ?? args?.path ?? "").trim();
		const content = String(args?.content ?? "");
		const { getOrImportWorkspace, writeVirtualFile } = await import("./virtual-workspace");
		const ws = await getOrImportWorkspace(ctx, rootPath);
		return writeVirtualFile(ws, filePath, content);
	}
	if (command === "workspace_replace") {
		...
		const { getOrImportWorkspace, replaceInVirtualFile } = await import("./virtual-workspace");
		const ws = await getOrImportWorkspace(ctx, rootPath);
		return replaceInVirtualFile(ws, filePath, oldText, newText);
	}
	if (command === "workspace_delete") {
		const rootPath = String(args?.rootPath ?? args?.cwd ?? ctx.workspaceRoot).trim();
		const filePath = String(args?.filePath ?? args?.path ?? "").trim();
		const { getOrImportWorkspace, deleteVirtualFile } = await import("./virtual-workspace");
		const ws = await getOrImportWorkspace(ctx, rootPath);
		return deleteVirtualFile(ws, filePath);
	}
	if (command === "workspace_push") {
		const rootPath = String(args?.rootPath ?? args?.cwd ?? ctx.workspaceRoot).trim();
		const createBackup = args?.createBackup !== false;
		const backupDir = args?.backupDir ? String(args.backupDir) : undefined;
		const { getOrImportWorkspace, pushVirtualWorkspaceToDisk } = await import("./virtual-workspace");
		const ws = await getOrImportWorkspace(ctx, rootPath);
		return await pushVirtualWorkspaceToDisk(ws, { createBackup, backupDir });
	}
