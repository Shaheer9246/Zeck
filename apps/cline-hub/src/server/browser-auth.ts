...
export function isAuthorizedBrowserRequest(
	...
	const reqHost =
		parseHeader(req.headers.get("x-forwarded-host")) ||
		parseHeader(req.headers.get("host"));
	const origin = parseOrigin(req.headers.get("origin"));
	const originUrl = origin ? new URL(origin) : undefined;
	const originHost = originUrl ? originUrl.host.toLowerCase() : undefined;

	// Check if this is a same-origin request or running within cloud environments (e.g. GitHub Codespaces, Cloud Run)
	const isSameOrigin = Boolean(
		reqHost && originHost && (reqHost === originHost || originHost.startsWith(reqHost))
	);
	const isTrustedCloudHost = Boolean(
		reqHost &&
			(reqHost.endsWith(".app.github.dev") ||
				reqHost.endsWith(".github.dev") ||
				reqHost.endsWith(".run.app") ||
				process.env.CODESPACES === "true")
	);

	if (reqHost && !allowedBrowserHosts(options).has(reqHost) && !isSameOrigin && !isTrustedCloudHost) {
		return false;
	}

	if (
		origin &&
		!allowedBrowserOrigins(options).has(origin) &&
		!isSameOrigin &&
		!isTrustedCloudHost
	) {
		return false;
