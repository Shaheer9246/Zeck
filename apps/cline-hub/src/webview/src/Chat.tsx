...
export default function Chat({
	...
	const activeSession = useMemo(
		() => (sessionId ? sessions.find((item) => item.sessionId === sessionId) : undefined),
		[sessionId, sessions],
	);
	const sessionTitle =
		sessionId && typeof activeSession?.title === "string"
			...
			<div className="relative flex h-screen flex-col overflow-hidden">
				<div className="flex items-center justify-between border-b px-4 py-3">
					<div className="flex min-w-0 items-center gap-2">
						...
						{sessionId && (activeSession?.workspaceRoot || activeSession?.cwd) ? (
							<span
								className="inline-flex max-w-48 items-center gap-1 truncate rounded border bg-muted/50 px-1.5 py-0.5 text-[11px] text-muted-foreground"
								title={`Session Folder: ${activeSession?.cwd || activeSession?.workspaceRoot || ""}`}
							>
								<FolderIcon className="size-3 shrink-0" />
								<span className="truncate">
									{(activeSession?.cwd || activeSession?.workspaceRoot || "").split("/").filter(Boolean).pop()}
								</span>
							</span>
						) : null}
						{sessionId ? (
				...
				<Conversation className="min-h-0 flex-1">
					<ConversationContent className="px-4 py-5">
						{isHydrating ? (
							...
							<div className="flex h-full min-h-[380px] flex-col items-center justify-center gap-6 py-12 text-center select-none">
								<div className="relative flex items-center justify-center">
									<div className="flex size-32 items-center justify-center rounded-2xl border border-border/50 bg-muted/20 p-5 shadow-sm">
										<img
											src="/icon.svg"
											alt="Cline Mascot"
											className="size-24 opacity-60 dark:opacity-80 dark:invert transition-transform hover:scale-105"
										/>
									</div>
								</div>
